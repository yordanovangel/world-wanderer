...
type ScoreResp = {
  score: number; reasoning: string; fraud_suspected: boolean;
  attempt_no: number; is_last_attempt: boolean; session_completed: boolean;
};
...
export default function QuestScoringPage() {
  ...
  useEffect(() => {
    ...
    if (!sessionId || !taskId || !submissionPath || !questId) {
      setError(t('quest.scoring.missing'));
      return;
    }
    (async () => {
      try {
        ...
        const result: ScoreResp = await scoreSubmission({
          session_id: sessionId, task_id: taskId, submission_path: submissionPath,
        });
        if (cancelled) return;

        const params = new URLSearchParams({
          session: sessionId, task: taskId,
          score: String(result.score), attempt: String(result.attempt_no),
        ...
        if (!cancelled) setError(e?.message || t('quest.scoring.errorTitle'));
      }
    })();
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-parchment-50">
      <div className="relative flex-1 overflow-hidden bg-ink-900">
        {previewUrl && <img src={previewUrl} alt={t('quest.scoring.yourPhoto')} className="absolute inset-0 h-full w-full object-contain" />}
        {!error && (
        ...
        {error ? (
          <>
            ...
            <button type="button" onClick={() => navigate(-1)}
              ...
              {t('quest.scoring.back')}
            </button>
          </>
        ) : (
          <p className="font-display text-xl text-ink-700">{t('quest.scoring.scanning')}</p>
        )}
      </div>
    </div>
  );
