...
function fmtTimestamp(iso: string | null, lang: string): string {
  if (!iso) return '—';
  const d = new Date(iso);
  return d.toLocaleString(lang.startsWith('en') ? 'en-US' : 'bg-BG', {
...
export default function SessionDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!s) {
          toast({ title: t('session.detail.notFound'), variant: 'destructive' });
          return;
        }
        if (cancelled) return;
        setSession(s);
        const [q, ts, subs, lb] = await Promise.all([
          fetchQuest(s.quest_id), fetchPublicTasks(s.quest_id), fetchSubmissions(s.id),
          fetchQuestLeaderboard(s.quest_id).catch(() => []),
        ]);
        if (cancelled) return;
        setQuest(q); setTasks(ts); setSubmissions(subs); setLeaderboard(lb);
        ...
        if (paths.length > 0) {
          try {
            ...
            Array.from(bestPerTask.values()).forEach((b, i) => { map[b.id] = urls[i]; });
            setThumbs(map);
          } catch { /* non-critical */ }
        }
      } catch (e: any) {
        toast({ title: t('session.detail.loadFailed'), description: e?.message, variant: 'destructive' });
      } finally {
  ...
  const attemptsByTask = useMemo(() => {
    ...
    for (const sub of submissions) map.set(sub.task_id, Math.max(map.get(sub.task_id) ?? 0, sub.attempt_no));
    return map;
  }, [submissions]);
  ...
  const totalScore = Array.from(bestByTask.values()).reduce((sum, s) => sum + (s.score ?? 0), 0);
  const duration = session.completed_at
    ? Math.max(0, Math.floor((new Date(session.completed_at).getTime() - new Date(session.started_at).getTime()) / 1000))
    : 0;

  return (
    <div className="mx-auto w-full max-w-md px-5 pb-10 pt-6">
      <Link to="/history" className="inline-flex items-center gap-1 text-sm text-ink-500 hover:text-ink-900">
        <ArrowLeft size={16} /> {t('session.detail.back')}
      </Link>
      ...
      <section className="mt-5 rounded-2xl border border-parchment-200 bg-white p-5 shadow-soft">
        <div className="flex items-start justify-between gap-3">
          <span className={`inline-block rounded-full px-2.5 py-1 text-xs font-medium ${STATUS_CLASS[session.status]}`}>
            {t(`history.status.${session.status}`)}
          </span>
          <div className="text-right">
            {isTreasure ? (
              <p className="font-mono-rq text-2xl font-semibold text-ink-900">{formatDuration(duration)}</p>
            ) : (
              <p className="font-mono-rq text-2xl font-semibold text-ink-900">
                {totalScore} <span className="text-base font-normal text-ink-500">{t('common.points')}</span>
              </p>
            )}
          </div>
        </div>
        <dl className="mt-4 space-y-1.5 text-sm">
          <div className="flex justify-between">
            <dt className="text-ink-500">{t('session.detail.started')}</dt>
            <dd className="font-mono-rq text-ink-700">{fmtTimestamp(session.started_at, i18n.language)}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-ink-500">{t('session.detail.finished')}</dt>
            <dd className="font-mono-rq text-ink-700">{fmtTimestamp(session.completed_at, i18n.language)}</dd>
          </div>
        </dl>
      </section>
      ...
      {quest.creator_id === user?.id && quest.share_token && (
        <button type="button" onClick={() => setShowShare(true)}
          ...
          <Share2 size={16} /> {t('session.detail.shareQuest')}
        </button>
      )}

      <section className="mt-6">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-ink-500">{t('session.detail.tasks')}</h2>
        <ul className="space-y-2.5">
          {tasks.map((task, i) => {
            ...
            return (
              <li key={task.id} className="flex gap-3 rounded-2xl border border-parchment-200 bg-white p-3 shadow-soft">
                <div className="h-16 w-16 flex-none overflow-hidden rounded-xl bg-parchment-100">
                  {thumb ? (
                    <img src={thumb} alt={t('session.detail.taskAlt', { n: i + 1 })} className="h-full w-full object-cover" loading="lazy" />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center text-xs text-ink-300">—</div>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-2">
                    <p className="truncate text-sm font-semibold text-ink-900">{i + 1}. {task.title}</p>
                    {best ? (
                      isTreasure ? (
                        <span className="flex-none rounded-full bg-forest-200 px-2 py-0.5 text-[11px] font-medium text-forest-700">{t('session.detail.found')}</span>
                      ) : (
                        <span className="flex-none font-mono-rq text-sm font-semibold text-ink-900">{best.score ?? 0}/{task.max_points}</span>
                      )
                    ) : (
                      <span className="flex-none text-[11px] text-ink-300">—</span>
                    )}
                  </div>
                  {best?.ai_reasoning && <p className="mt-1 line-clamp-2 text-xs text-ink-500">{best.ai_reasoning}</p>}
                  {attempts > 1 && (
                    <span className="mt-1.5 inline-block rounded-full bg-parchment-100 px-2 py-0.5 text-[10px] font-medium text-ink-500">
                      {t('session.detail.attempts', { count: attempts })}
                    </span>
                  )}
                </div>
              </li>
      ...
      {leaderboard.length > 1 && (
        <section className="mt-8">
          <h2 className="mb-3 flex items-center gap-1.5 text-sm font-semibold uppercase tracking-wider text-ink-500">
            <Trophy size={14} /> {t('session.detail.leaderboard')}
          </h2>
            ...
            {leaderboard.slice(0, 10).map((row) => {
              ...
              return (
                <li key={row.session_id}
                  ...
                  <span className="w-6 flex-none text-center font-mono-rq text-sm font-semibold text-ink-700">{row.rank ?? '—'}</span>
