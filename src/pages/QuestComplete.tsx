...
import {
  fetchPublicTasks, fetchQuest, fetchSession, fetchSubmissions,
  type PublicTask, type Quest, type Submission,
...
export default function QuestCompletePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [q, ts, session] = await Promise.all([
          fetchQuest(questId), fetchPublicTasks(questId), fetchSession(sessionId),
        ]);
        const subs = await fetchSubmissions(sessionId);
        if (cancelled) return;
        setQuest(q); setTasks(ts); setSubmissions(subs);
        if (session) {
  ...
  const max = tasks.reduce((s, tt) => s + tt.max_points, 0);

  if (loading) {
    return <div className="mx-auto w-full max-w-md px-5 pt-10 text-center text-ink-500">{t('quest.complete.loading')}</div>;
  }
  ...
  return (
    <div className="mx-auto w-full max-w-md px-5 pb-12 pt-8 rq-grain">
      <header className="text-center">
        ...
        <h1 className="mt-3 font-display text-[40px] leading-tight text-ink-900">{t('quest.complete.title')}</h1>
        {quest && <p className="mt-1 text-base text-ink-500">{quest.title}</p>}
      </header>

      <section className="mt-8 grid grid-cols-2 gap-3">
        <Stat label={t('quest.complete.score')} value={`${total}/${max || '—'}`} accent="ochre" />
        <Stat label={t('quest.complete.duration')} value={formatDuration(duration)} />
      </section>

      <section className="mt-8">
        <h2 className="mb-3 px-1 text-xs font-semibold uppercase tracking-wider text-ink-500">{t('quest.complete.perTask')}</h2>
        <ol className="space-y-2">
          {progress.states.map((s, i) => (
            <li key={s.task.id} className="flex items-center gap-3 rounded-xl border border-parchment-200 bg-white p-3 shadow-soft">
              ...
              <p className="min-w-0 flex-1 truncate text-sm font-medium text-ink-900">{s.task.title}</p>
              <span className="font-mono-rq text-sm text-ink-500">{s.bestScore}/{s.task.max_points}</span>
            </li>
          ))}
        </ol>
      </section>
      ...
      <div className="mt-8 space-y-3">
        ...
        <Link to="/history"
          ...
          <NotebookPen size={18} /> {t('quest.complete.toMyGames')}
        </Link>
      </div>
    </div>
  );
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: 'ochre' }) {
  return (
    <div className="rounded-2xl border border-parchment-200 bg-white p-4 text-center shadow-soft">
      ...
      <p className={`mt-1 font-display text-[28px] leading-none ${accent === 'ochre' ? 'text-ochre-700' : 'text-ink-900'}`}>{value}</p>
    </div>
  );
}
