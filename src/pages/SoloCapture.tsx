...
export default function SoloCapturePage() {
  ...
  const addSlot = async (blob: Blob) => {
    ...
    try {
      ...
      toast({ title: t('solo.uploadFailed'), description: e?.message, variant: 'destructive' });
    }
  };
  ...
  const onGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const paths = slots.filter((s): s is Slot & { storagePath: string } => !!s.storagePath).map((s) => s.storagePath);
      const result = await generateSoloQuest({ source_paths: paths });
      navigate(`/quest/${result.quest_id}/intro`, { replace: true });
    } catch (e: any) {
      toast({ title: t('solo.generateFailed'), description: e?.message, variant: 'destructive' });
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="mx-auto w-full max-w-md px-5 pb-10 pt-6">
      ...
      <header className="mt-4">
        <h1 className="font-display text-[22px] text-ink-900">{t('solo.captureTitle')}</h1>
        <p className="mt-1 text-sm text-ink-500">{t('solo.captureSubtitle')}</p>
      </header>
      ...
      <div className="mt-6 grid grid-cols-3 gap-3">
        {Array.from({ length: MAX_SLOTS }).map((_, i) => {
          ...
          if (!slot) {
            ...
            return <CaptureSlotEmpty key={i} disabled={!isNext} onCapture={addSlot} />;
          }
          return <FilledSlot key={i} slot={slot} onRemove={() => removeSlot(i)} />;
        })}
      </div>

      <p className="mt-3 text-center text-xs text-ink-500">{t('solo.uploaded', { done: uploadedCount, total: MAX_SLOTS })}</p>
      ...
      <button type="button" onClick={onGenerate} disabled={!ready}
        ...
        <Sparkles size={18} /> {t('solo.create')}
      </button>
...
function CaptureSlotEmpty({ onCapture, disabled }: { onCapture: (b: Blob) => void; disabled: boolean }) {
  const [hidden, setHidden] = useState(false);
  return (
    <div className={`flex aspect-square items-center justify-center rounded-2xl border-2 border-dashed border-parchment-200 bg-white/40 ${disabled ? 'opacity-40' : ''}`}>
      {!hidden && (
        <div className="scale-90">
          <CameraCapture label="" onCapture={(b) => { setHidden(true); onCapture(b); setHidden(false); }} disabled={disabled} />
        </div>
      )}
    </div>
  );
...
function FilledSlot({ slot, onRemove }: { slot: Slot; onRemove: () => void }) {
  ...
  return (
    <div className="relative aspect-square overflow-hidden rounded-2xl bg-parchment-100 shadow-soft">
      <img src={slot.previewUrl} alt={t('image.preview')} className="h-full w-full object-cover" draggable={false} />
      ...
      <button type="button" onClick={onRemove} aria-label={t('solo.removeAria')}
...
function GeneratingOverlay() {
  const { t } = useTranslation();
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-parchment-50/95 backdrop-blur" role="status" aria-live="polite">
      <Compass size={64} className="animate-spin text-terracotta-500" style={{ animationDuration: '2.4s' }} />
      <p className="font-display text-2xl text-ink-900">{t('solo.analyzing')}</p>
      <p className="text-sm text-ink-500">{t('solo.mayTakeMinute')}</p>
    </div>
  );
}
