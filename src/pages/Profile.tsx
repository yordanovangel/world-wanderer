...
import { ChevronRight, LogOut, Moon, Trash2, Languages } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [stats, setStats] = useState<{ played: number; created: number; totalScore: number } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [s, c] = await Promise.all([fetchUserStats(user.id), fetchCreatedQuestsAll(user.id)]);
        if (cancelled) return;
        setStats(s);
        setCreated(c);
      } catch (e: any) {
        toast({ title: t('profile.loadFailedTitle'), description: e?.message, variant: 'destructive' });
      }
    })();
  ...
  const handleClearLocal = () => {
    if (!confirm(t('profile.confirmClear'))) return;
    Object.keys(localStorage)
      .filter((k) => k.startsWith('rq_') && k !== 'rq_auth_token' && k !== 'rq_auth_user' && k !== 'rq_lang')
      .forEach((k) => localStorage.removeItem(k));
    toast({ title: t('profile.clearedToast') });
  };
  ...
  const MODE_LABEL_KEY = (m: 'solo' | 'multiplayer' | 'treasure_hunt') => t(`mode.${m}`);

  return (
    <div className="mx-auto w-full max-w-md px-5 pb-10 pt-6">
      <h1 className="font-display text-[28px] leading-tight text-ink-900">{t('profile.title')}</h1>
      ...
      <section className="mt-6 flex items-center gap-4 rounded-2xl border border-parchment-200 bg-white p-5 shadow-soft">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 font-mono-rq text-[11px] text-ink-300">{user?.id.slice(0, 8)}</p>
        </div>
      </section>
      ...
      {stats && (
        <section className="mt-3 grid grid-cols-3 gap-2 rounded-2xl border border-parchment-200 bg-white p-4 shadow-soft">
          <div className="text-center">
            ...
            <p className="text-[11px] uppercase tracking-wider text-ink-500">{t('profile.played')}</p>
          </div>
          <div className="text-center">
            <p className="font-mono-rq text-2xl font-semibold text-ink-900">{stats.created}</p>
            <p className="text-[11px] uppercase tracking-wider text-ink-500">{t('profile.created')}</p>
          </div>
          <div className="text-center">
            <p className="font-mono-rq text-2xl font-semibold text-ink-900">{stats.totalScore}</p>
            <p className="text-[11px] uppercase tracking-wider text-ink-500">{t('profile.xp')}</p>
          </div>
        </section>
      )}
      ...
      <section className="mt-6">
        <div className="mb-2 flex items-baseline justify-between">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-ink-500">{t('profile.myQuests')}</h2>
            ...
            {recentCreated.map((q) => (
              <li key={q.id}>
                ...
                >
                  ...
                  {q.status === 'draft' ? (
                    <span className="text-xs font-medium text-terracotta-500">{t('profile.continueArrow')}</span>
                  ) : (
      ...
      <section className="mt-8">
        <h2 className="mb-2 text-sm font-semibold uppercase tracking-wider text-ink-500">{t('profile.settings')}</h2>

        {/* Language switcher */}
        <div className="mb-2 flex items-center gap-3 rounded-xl border border-parchment-200 bg-white p-3.5 shadow-soft">
          <Languages size={18} className="text-ink-500" />
          <span className="flex-1 text-sm text-ink-900">{t('profile.language')}</span>
          <div className="inline-flex overflow-hidden rounded-lg border border-parchment-200">
            <button
              type="button"
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold ${lang === 'en' ? 'bg-terracotta-500 text-parchment-50' : 'bg-white text-ink-700 hover:bg-parchment-100'}`}
            >
              EN
            </button>
          </div>
        ...
        <ul className="space-y-1.5">
          <li>
            <button
              type="button"
              onClick={() => toast({ title: t('profile.darkSoonTitle'), description: t('profile.darkSoonDesc') })}
            ...
            >
              ...
              <span className="flex-1 text-sm text-ink-900">{t('profile.darkMode')}</span>
              <span className="text-[11px] uppercase tracking-wider text-ink-300">{t('profile.soon')}</span>
            </button>
          </li>
          <li>
            <button
            ...
            >
              ...
              <span className="flex-1 text-sm text-ink-900">{t('profile.clearLocal')}</span>
            </button>
          </li>
          <li>
            <button
            ...
            >
              ...
              <span className="flex-1 text-sm font-medium text-danger-600">{t('profile.logout')}</span>
            </button>
          </li>
        </ul>
        <p className="mt-6 text-center font-mono-rq text-[11px] text-ink-300">Reality Quest {APP_VERSION}</p>
      </section>
    </div>
  );
}
