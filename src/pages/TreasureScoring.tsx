...
type CompareResp = { match: boolean; user_hint: string; attempt_no: number; session_completed: boolean };
...
export default function TreasureScoringPage() {
  ...
  useEffect(() => {
    ...
    if (!sessionId || !taskId || !submissionPath || !questId) {
      setError(t('treasure.scoring.missing'));
      return;
    }
    (async () => {
      try {
        const signResp = await invokeFn<{ urls: string[] }>('sign-download-urls', {
          bucket: SUBMISSION_BUCKET, paths: [submissionPath], ttl_sec: 600,
        });
        if (cancelled) return;
        if (signResp.urls?.[0]) setPreviewUrl(signResp.urls[0]);

        const r = await compareTreasureSubmission({ session_id: sessionId, task_id: taskId, submission_path: submissionPath });
        if (cancelled) return;
        setResult(r);

        if (r.session_completed) {
          setTimeout(() => navigate(`/quest/${questId}/complete?session=${sessionId}`, { replace: true }), 1600);
        } else if (r.match) {
          setTimeout(() => navigate(`/quest/${questId}/treasure-play?session=${sessionId}`, { replace: true }), 1400);
        }
      } catch (e: any) {
        if (!cancelled) setError(e?.message || t('treasure.scoring.errorFallback'));
      }
    })();
    return () => { cancelled = true; };
  }, [questId, sessionId, taskId, submissionPath, navigate, t]);

  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-parchment-50">
      <div className="relative flex-1 overflow-hidden bg-ink-900">
        {previewUrl && <img src={previewUrl} alt={t('treasure.scoring.yourPhoto')} className="absolute inset-0 h-full w-full object-contain" />}
        {!error && !result && (
          <div className="pointer-events-none absolute left-0 right-0 h-1.5 bg-terracotta-500/90 shadow-[0_0_24px_rgba(217,118,63,0.6)]"
            style={{ animation: 'rq-scan 1500ms ease-in-out infinite' }} />
        )}
        <style>{`@keyframes rq-scan { 0% { bottom: 0%; opacity: 0; } 10% { opacity: 1; } 90% { opacity: 1; } 100% { bottom: 100%; opacity: 0; } }`}</style>
      </div>
        ...
        {error ? (
          <>
            ...
            <button type="button" onClick={() => navigate(-1)}
              ...
              {t('treasure.scoring.back')}
            </button>
          </>
        ) : !result ? (
          <p className="font-display text-xl text-ink-700">{t('treasure.scoring.scanning')}</p>
        ) : result.match ? (
          <>
            <p className="font-display text-2xl text-forest-700">{t('treasure.scoring.match')}</p>
            <p className="text-sm text-ink-500">{result.user_hint}</p>
          </>
        ) : (
          <>
            <p className="font-display text-2xl text-danger-600">{t('treasure.scoring.noMatch')}</p>
            ...
            <button type="button"
              ...
              <RefreshCcw size={16} /> {t('treasure.scoring.tryAgain')}
            </button>
          </>
        )}
      </div>
